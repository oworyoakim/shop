<template>
    <div class="items mt-2">
        <Spinner v-if="isLoading || !itemsInfo" />
        <template v-else>
            <div class="card">
                <div class="card-header with-border">
                    <h3 class="card-title"><i class="fa fa-shopping-basket"></i> &nbsp;Items</h3>
                ...
                <div class="card-footer text-center">
                    <Pagination :items="itemsInfo" @gotoPage="getItems" />
                </div>
                <ItemForm @ITEM_SAVED="getItems()" />
...
import ItemForm from "../../components/Stocks/ItemForm";
export default {
    components: {Pagination, Spinner, ItemForm},
    mounted() {
        this.getItems();
    ...
    methods: {
        ...
        async getItems(page = 0) {
            try {
                ...
                await this.$store.dispatch('GET_ITEMS', this.filters);
        ...
        async deleteItem(item) {
            try {
                let isConfirmed = await this.$store.dispatch('CONFIRM_ACTION',{
                    title: "Are you sure?",
                    text: `You will delete ${item.name}!`,
                    icon: 'warning',
        ...
        async printItemBarcode(barcode){
            try{
                ...
                let printUrl = `/items/print-barcode/${this.barcode}?count=${this.barcodeCount}`;
